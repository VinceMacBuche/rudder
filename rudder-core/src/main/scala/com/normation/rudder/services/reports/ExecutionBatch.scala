          ...
          case (None, Some(configs)) =>
            if(configs.isEmpty) {
              ...
              complianceMode match {
                case ChangesOnly(heartbeatFrequency) => NoReportInInterval(currentConfig)
                case FullCompliance =>
          ...
          case (Some(AgentRun(AgentRunId(_, t), None, _)), Some(configs)) =>
            if(configs.isEmpty) {
              ...
              if(oldest.creation.plus(graceDuration).isBefore(t) ) {
                ...
                complianceMode match {
                  case ChangesOnly(heartbeatFrequency) => CheckChanges(t, current)
                  case FullCompliance =>
              ...
              findVersionById(rv, configs) match {
                ...
                case Some(v) => //nominal case !
                  ...
                  v.endOfLife match {
                    case None =>
                      complianceMode match {
                        case ChangesOnly(heartbeatFrequency) => CheckChanges(t, v)
                        case FullCompliance =>
