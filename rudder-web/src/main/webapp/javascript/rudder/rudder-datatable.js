...
function createRuleTable(gridId, data, needCheckbox, isPopup, allCheckboxCallback, contextPath, refresh) {
  ...
  var recentChanges = {
    ...
    , "fnCreatedCell" : function (nTd, sData, oData, iRow, iCol) {
        var elem = callbackElement(oData);
        elem.append('<canvas class="recentChanges" width="150" height="30"></canvas>'); //chart drawn in fnRowCallback
        $(nTd).empty();
        $(nTd).prepend(elem);
      }
  };
...
function buildComplianceBar(compliance) {
  ...
  if(notApplicable != 0) {
    content.append('<div class="progress-bar progress-bar-notapplicable" style="width:'+notApplicable+'%" title="Not applicable: '+notApplicable+'%">'+notApplicable+'%</div>')
  }
  ...
  if(pending != 0) {
    content.append('<div class="progress-bar progress-bar-pending active progress-bar-striped" style="width:'+pending+'%" title="Applying: '+pending+'%">'+pending+'%</div>')
  }
  var noreport = compliance[5]
  if(noreport != 0) {
    content.append('<div class="progress-bar progress-bar-noanswer" style="width:'+noreport+'%" title="No Report: '+noreport+'%">'+noreport+'%</div>')
  }
  ...
  if(error != 0) {
    content.append('<div class="progress-bar progress-bar-error" style="width:'+error+'%" title="Error: '+error+'%">'+error+'%</div>')
  }
