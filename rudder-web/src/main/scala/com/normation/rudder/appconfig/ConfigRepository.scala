...
trait ConfigRepository {
  ...
  def getConfigParameters(): Box[Seq[GlobalParameter]]

  def saveConfigParameter(parameter: GlobalParameter): Box[GlobalParameter]
...
class LdapConfigRepository(
  ...
  def saveConfigParameter(parameter: GlobalParameter) = {
    for {
      con        <- ldap
      paramEntry =  mapper.rudderConfig2Entry(parameter)
      result     <- con.save(paramEntry) ?~! "Error when saving parameter entry in repository: %s".format(paramEntry)
    } yield {
      parameter
    }
  }
