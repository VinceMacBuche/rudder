...
class RestArchiving(
  ...
  private[this] def restoreLatestArchive(req:Req, list:() => Box[Map[DateTime, GitArchiveId]], restore:(GitCommitId,ModificationId,EventActor,Option[String],Boolean) => Box[GitCommitId], archiveType:String) = {
    (for {
      archives   <- list()
      (date,tag) <- Box(archives.toList.sortWith { case ( (d1,_), (d2,_) ) => d1.isAfter(d2) }.headOption) ?~! "No archive is available"
      restored   <- restore(tag.commit,newModId,RestUtils.getActor(req),Some("estore latest archive required from REST API"),false)
    } yield {
      restored
    }) match {
      case eb:EmptyBox =>
  ...
  private[this] def restoreLatestCommit(req:Req, restore: (ModificationId,EventActor,Option[String],Boolean) => Box[GitCommitId], archiveType:String) = {
    (for {
      restored   <- restore(newModId,RestUtils.getActor(req),Some("Restore archive from latest commit on HEAD required from REST API"), false)
    } yield {
      restored
    }) match {
      case eb:EmptyBox =>
  ...
  private[this] def restoreByDatetime(req:Req, list:() => Box[Map[DateTime, GitArchiveId]], restore:(GitCommitId,ModificationId,EventActor,Option[String],Boolean) => Box[GitCommitId], datetime:String, archiveType:String) = {
    (for {
      ...
      restored   <- restore(tag.commit,newModId,RestUtils.getActor(req),Some("Restore archive for date time %s requested from REST API".format(datetime)),false)
    } yield {
      restored
    }) match {
      case eb:EmptyBox =>
