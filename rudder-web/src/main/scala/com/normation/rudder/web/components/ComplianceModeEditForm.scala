...
class ComplianceModeEditForm (
  ...
  // Html template
  def templatePath = List("templates-hidden", "components", "ComponentAgentHeartbeat")
  def template() =  Templates(templatePath) match {
     case Empty | Failure(_,_,_) =>
       error(s"Template for Compliance mode configuration not found. I was looking for ${templatePath.mkString("/")}.html")
     case Full(n) => n
  }
  def agentScheduleTemplate = chooseTemplate("property", "agentHeartbeat", template)
    ...
    val transform = (for {
      ...
      console.log(scope);
          scope.$$apply(function(){
            scope.init("${complianceMode}", ${frequency}, ${globalRun} ,${callback.toJsCmd}, "${S.contextPath}");
          } );
      """
    }) match {
      case eb:EmptyBox =>
