...
class Archives extends DispatchSnippet with Loggable {
  ...
  private[this] def actionFormBuilder(
    ...
    , restoreFunction           : (GitCommitId, ModificationId, EventActor, Option[String], Boolean) => Box[GitCommitId] //the actual logic to execute the action
    ...
    def restore(): JsCmd = {
      ...
      selectedCommitId match {
        ...
        case Some(commit) => restoreFunction(commit, ModificationId(uuidGen.newUuid), CurrentUser.getActor, Some("User requested archive restoration"), false) match {
