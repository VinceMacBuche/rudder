 ...
 private[this] def mapSimpleDiff[T](opt:Option[SimpleDiff[T]], id: DirectiveId) = opt.map { diff =>
  ...
  private[this] def displayRollbackDetails(rollbackInfo:RollbackInfo,id:Int) = {
    ...
    <div class="evloglmargin">
      ...
      <b>This is rollback to {rollbackInfo.rollbackType} event {
        SHtml.a(() => JsRaw("""$('#%s').dataTable().fnFilter("%s|%s",0,true,false); $("#cancel%s").show();""".format(gridname,rollbackInfo.target.id,id,id)), Text(rollbackInfo.target.id.toString))
        }
      </b>
      <b>The following events were rollbacked and are not applied anymore by the configuration policy</b>
      <table id={"rollbackTable%s".format(id)}>
      <thead> <th>Id</th> <th>Type</th> <th>Actor</th> <th>Date</th>  </thead>
      <tbody>
      {rollbackedEvents.map{ ev =>
        <tr>
          <td> 
            {SHtml.a(() => JsRaw("""$('#%s').dataTable().fnFilter("%s|%s",0,true,false); $("#cancel%s").show();""".format(gridname,ev.id,id,id)), Text(ev.id.toString))}
          </td>
          <td>{ev.eventType} </td>
          <td>{ev.author} </td>
          <td>{ev.date} </td>
      ...
      {SHtml.ajaxButton("Show all events", () =>
        Run("""$('#%s').dataTable().fnFilter("%s|%s",0,true,false); $("#cancel%s").show();""".format(gridname,rollbackedEvents.map(_.id).mkString("|"),id,id)) ) }
        {SHtml.ajaxButton("Cancel", () =>
        Run("""$('#%s').dataTable().fnFilter("",0,true,false); $("#cancel%s").hide();""".format(gridname,id)),("id","cancel%s".format(id)),("style","display:none;") ) }
    </div> ++ Script(JsRaw("""$('#rollbackTable%s').dataTable({
            "asStripeClasses": [ 'color1', 'color2' ],
            "bAutoWidth": false,
            "bFilter" :false,
            "bPaginate" :true,
