...
class EventListDisplayer(
  ...
  def display(events:Seq[EventLog]) : NodeSeq  = {

    def getLastEvents = {
      repos.getEventLogByCriteria(None, Some(1000), Some("id DESC")) match {
        case Full(events ) =>
          JsTableData(events.map(EventLogLine(_)).toList)
        case _ => JsTableData(Nil)
      }
    }
    val refresh = AnonFunc( SHtml.ajaxInvoke(() => JsRaw(s"refreshTable('${gridName}',${getLastEvents.json.toJsCmd})")))
