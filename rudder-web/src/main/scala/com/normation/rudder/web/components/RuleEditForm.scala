...
/**
 * This class create RuleFormFields from a Rule and some repositories
 * The main goal is to share those fields with components that uses them (RuleEditForm, RuleCompliance)
 */
case class RuleFormFields(
    rule : Rule
  , categoryHierarchyDisplayer : CategoryHierarchyDisplayer
  , roCategoryRepository : RoRuleCategoryRepository
) {

  ///////////// fields for Rule settings ///////////////////
  val crName = new WBTextField("Name", rule.name) {
