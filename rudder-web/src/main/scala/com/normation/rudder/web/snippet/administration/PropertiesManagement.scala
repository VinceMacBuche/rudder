...
import com.normation.rudder.web.components.AgentModeEditForm
...
class PropertiesManagement extends DispatchSnippet with Loggable {
  ...
  def agentModeEditForm = {
    val globalMode = configService.rudder_global_agent_mode()
    new AgentModeEditForm(
        globalMode
      , agentMode =>  {  configService.set_rudder_global_agent_mode(agentMode ,CurrentUser.getActor,genericReasonMessage)}
      , () => startNewPolicyGeneration
      , globalMode
    )
  }
  ...
  def agentModeConfiguration = agentModeEditForm.agentModeConfiguration
