...
class RuleGrid(
  ...
  def dispatch = {
    case "rulesGrid" => { _:NodeSeq => rulesGrid()}//getAllNodeInfos(), getFullNodeGroupLib(), getFullDirectiveLib(), recentChanges.getChangesByInterval(2)) }
  }

  def jsVarNameForId(tableId:String) = "oTable" + tableId

  def rulesGridWithUpdatedInfo(popup: Boolean = false, linkCompliancePopup:Boolean = true) = {
    rulesGrid(/*getAllNodeInfos(), getFullNodeGroupLib(), getFullDirectiveLib(), recentChanges.getChangesByInterval(2),*/ popup, linkCompliancePopup)
  }
