      ...
      for {
        res <- calls.toList.traverse(block =>
          for {
            ...
            val compositionRule = block.compositionRule match {
              case WorstReport => "worst"
              case SumReport => "sum"
              case ComponentReport(component) => s"component:${component}"
            }
            <SECTION component="true" multivalued="true" name={component} composition={compositionRule}>
              <REPORTKEYS>
                {childs}
              </REPORTKEYS>
            </SECTION>
...
class ClassicTechniqueWriter(basePath : String, parameterTypeService: ParameterTypeService) extends AgentSpecificTechniqueWriter {
  ...
  def methodNeedReporting(methods : Map[BundleName, GenericMethod])(call : Method) : Boolean = {
    call match {
  ...
  def needReportingBundle(technique : Technique, methods : Map[BundleName, GenericMethod]) = technique.methodCalls.exists(methodNeedReporting(methods))

  def canonifyCondition(methodCall: MethodCall, parentBlock : List[MethodBlock]) = {
    val condition = parentBlock.map(_.condition).filterNot(truthyCondition).mkString("",".", s".${methodCall.condition}")
  ...
  def writeAgentFiles( technique : Technique, methods : Map[BundleName, GenericMethod] )  : IOResult[Seq[String]] = {
    ...
    def bundleMethodCall( parentBlocks : List[MethodBlock])(method : Method) : List[String] = {
      method match {
...
class DSCTechniqueWriter(
  ...
  def writeAgentFiles(technique : Technique, methods : Map[BundleName, GenericMethod] ): IOResult[Seq[String]] = {
    ...
    def toDscFormat(parentBlocks: List[MethodBlock])(method : Method) : PureResult[List[String]] = {
      method match {
        case call : MethodCall =>
          if (call.methodId.value.startsWith("_")) {
            Right(Nil)
