...
object GlobalScoreRepositoryImpl {
  ...
  implicit val getNoDetailsScore: Get[List[NoDetailsScore]] = Get[String].temap(_.fromJson[List[NoDetailsScore]])
  implicit val putNoDetailsScore: Put[List[NoDetailsScore]] = Put[String].contramap(_.toJson)
  ...
  implicit val globalScoreRead: Read[GlobalScore] = {
    Read[(ScoreValue, String, List[NoDetailsScore])].map {
      d: (ScoreValue, String, List[NoDetailsScore]) =>
...
class GlobalScoreRepositoryImpl(doobie : Doobie) extends GlobalScoreRepository {
  ...
  def save(nodeId: NodeId, globalScore: GlobalScore): IOResult[(NodeId,GlobalScore)]       = {
    ...
    transactIOResult(s"error when inserting global score for node '${nodeId.value}''")(xa => query.update.run.transact(xa)).map(_ => (nodeId,globalScore))
  }

  override def getAll(): IOResult[Map[NodeId, GlobalScore]] = {
    val q = sql"select nodeId, score, message, details from GlobalScore"
    transactIOResult(s"error when getting global scores for node")(xa => q.query[(NodeId,GlobalScore)].to[List].transact(xa)).map(_.groupMapReduce(_._1)(_._2){ case (_,res) => res})
  }
